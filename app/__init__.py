...
def create_app():
    ...
    # Base de datos
    # En Vercel se utiliza /tmp porque es el espacio temporal escribible.
    # En la computadora se utiliza la carpeta instance.
...
def _migrar_columnas_faltantes():
    """
    Agrega columnas nuevas a bases de datos creadas
    con una versión anterior del modelo.
    """
    ...
    columnas = [
        ...
        for row in db.session.execute(
            text("PRAGMA table_info(incidencia_factura)")
        ).fetchall()
    ]
    ...
    if columnas and "resuelto_por_id" not in columnas:
        db.session.execute(
            text(
                "ALTER TABLE incidencia_factura "
                "ADD COLUMN resuelto_por_id INTEGER"
            )
        )
